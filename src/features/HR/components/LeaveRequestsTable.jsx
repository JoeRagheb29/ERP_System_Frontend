...
import { faPen, faTrash, faSpinner, faExclamationCircle, faClipboardList, faEye, faSort, faSortUp, faSortDown, faArrowRotateRight } from '@fortawesome/free-solid-svg-icons';
...
export default function LeaveRequestsTable({
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      {loading && (
        <div className="p-6 space-y-3" role="status" aria-label="Loading leave requests">
          {[1, 2, 3, 4, 5].map((i) => (
      ...
      {!loading && error && (
        <div className="flex flex-col items-center justify-center py-14 px-6">
          ...
          <p className="text-sm font-semibold text-slate-800 mb-1">Failed to load leave requests</p>
                ...
                <tr>
                  <td colSpan={SORT_COLUMNS.length + 1 + (enableSelection ? 1 : 0)}>
                    <div className="flex flex-col items-center justify-center py-14">
                      ...
                      <p className="text-sm font-semibold text-slate-700 mb-1">No leave requests found</p>
                      <p className="text-xs text-slate-400 max-w-xs text-center">
                        There are no leave requests matching your current filters. Try adjusting or clearing them.
                      </p>
                    </div>
                  </td>
                </tr>
                ...
                records.map((rec, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5 text-sm text-slate-500 whitespace-nowrap">
                      {LEAVE_TYPE_LABELS[rec.leave_type] || rec.leave_type}
                    </td>
                    <td className="px-4 py-3.5 text-sm text-slate-500 whitespace-nowrap">{formatDate(rec.start_date)}</td>
                    <td className="px-4 py-3.5 text-sm text-slate-500 whitespace-nowrap hidden md:table-cell">{formatDate(rec.end_date)}</td>
                    <td className="px-4 py-3.5 text-sm text-slate-500 whitespace-nowrap text-center">{rec.total_days}</td>
                    <td className="px-4 py-3.5 whitespace-nowrap">
                      <StatusBadge status={rec.status} />
                    </td>
                    <td className="px-4 py-3.5 text-sm text-slate-500 whitespace-nowrap">{formatDateTime(rec.requested_at)}</td>
                        ...
                        {canEdit && (
                          <>
                            ...
                            <button
                              ...
                              aria-label={`Delete leave request for ${rec.employee_name}`}
                            >
